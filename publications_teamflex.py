...
def check_input(publication_id):
    if 'cockpit' in publication_id:
        return 'Plak niet de hele url, alleen de publicationId (bijvoorbeeld: publications-123-A)'
...
def get_url(publication_id):
    limit = 1000
    url_teamflex = f'https://teamflex.recruitnowcockpit.nl/jobsite/api/scripts/jobboard-search-results.js?sort=Created&sortDirection=Descending&limit={limit}&filters=agencies-1-C'

    json_data_Teamflex = get_json(url_teamflex)

    job_details_Teamflex = extract_job_details(json_data_Teamflex, 'Teamflex')

    for job in job_details_Teamflex:
